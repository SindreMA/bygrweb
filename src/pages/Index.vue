<template>
  <q-page class="minorMargin minorPadding">
    ...
    <div v-if="items" class="minorPadding">
      <div v-for="item in items.filter(x=> {return filterType == null || x.layoutType == filterType})" :key="item.id">
  ...
  methods: {
    ...
    filterFn (val, update, abort) {
      ...
      update(() => {
        ...
        this.filterOptions = this.getTypes.filter(v => v.toLowerCase().indexOf(needle) > -1)
      })
    },
  },
  created () {
    var vm = this
    axios.get("https://bygrapi.sindrema.com/api/buyer-info").then(x=> {
      if (x.data) {
        vm.items = x.data
      }
    }).catch(x=> {
